...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    deno = lambda row: np.sqrt(np.dot(row, row.T))
    deno = np.apply_along_axis(deno, 1, x)[:, None]
    x /= deno
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    outputs = softmax(prob)
    cost = -np.squeeze(np.log(outputs[target]))

    dz = outputs[:, None]
    dz[target, 0] -= 1

    gradPred = np.sum(dz * outputVectors, 0)
    grad = np.dot(dz, predicted[None, :])
    ### END YOUR CODE
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    ### YOUR CODE HERE
    input = np.zeros(inputVectors.shape[1])
    input_indices = [tokens[w] for w in contextWords]
    for ii in input_indices:
        input += inputVectors[ii]
    target = tokens[currentWord]
    cost, gradPred, grad = word2vecCostAndGradient(input, target, outputVectors, dataset)
